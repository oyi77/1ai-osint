...
def generate_html_report(result: DeepScanResult) -> str:
    ...
    social_profiles = [i for i in result.identifiers if i.id_type == IdentifierType.SOCIAL_PROFILE]
...
def generate_pdf_report(result: DeepScanResult) -> bytes:
    ...
    try:
        ...
        elements.append(Paragraph(f"Deep Scan Report: {result.target}", styles['Title']))
