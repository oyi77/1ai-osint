...
class WatchlistTarget(BaseModel):
    ...
    target_type: str = Field(
        ..., description="One of: email, username, domain, phone, wallet"
    )
    tags: list[str] = Field(default_factory=list)
    interval_hours: int = Field(
        default=24, ge=1, description="Hours between re-scans"
    )
    last_scan: Optional[datetime] = Field(
        default=None, description="When the target was last scanned"
    )
...
class ChangeEvent(BaseModel):
    ...
    old_value: Optional[str] = Field(
        default=None, description="Previous value (if applicable)"
    )
    new_value: Optional[str] = Field(
        default=None, description="Current value (if applicable)"
    )
    source_module: str = Field(
        ..., description="The module that produced the change data"
    )
    timestamp: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc)
    )
    severity: ChangeSeverity = Field(
        default=ChangeSeverity.INFO, description="Computed severity"
    )
    description: str = Field(
        default="", description="Human-readable change summary"
    )
    confidence: float = Field(
        default=0.5, ge=0.0, le=1.0, description="Confidence in this change"
    )
...
class AlertRule(BaseModel):
    ...
    target_pattern: str = Field(
        ..., description="Glob or regex pattern to match target names"
    )
    condition: str = Field(
        ...,
