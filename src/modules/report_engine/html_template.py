...
def _id_section(title: str, items: list[str], icon: str) -> str:
    ...
    more = f'<p style="color:#5a6a7a;margin-top:10px">+ {len(items)-30} more</p>' if len(items) > 30 else ""
...
<div class="meta-line"><span>Module: {_esc(f.module)}</span><span>ID: {_esc(f.id[:8])}</span></div>
    ...
    more = f'<p style="color:#5a6a7a;margin-top:10px">+ {len(findings)-50} more</p>' if len(findings) > 50 else ""
    return f'''<div class="section">
