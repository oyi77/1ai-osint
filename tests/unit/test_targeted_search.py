...
from src.modules.crypto.balance.chains import ETHEREUM, BITCOIN, SOLANA, ALL_CHAINS
...
class TestTargetedScanToScanResult:
    ...
    def test_converts_partial_status(self):
        ...
        targeted = TargetedScanResult(
            ...
            findings=[
                Finding(
                    id="f1", module="crypto_balance", title="test",
...
class TestKnownMnemonicLookup:
    ...
    async def test_account_range(self):
        ...
        with patch(
            ...
            # 3 account indices * 1 chain (ETH has 1 derivation path)
...
class TestFilteredRandomScan:
    ...
    async def test_filters_by_min_balance(self):
        ...
        low_balance = BalanceResult(
            address="0x123", chain="Ethereum", symbol="ETH",
            balance=0.0001, balance_raw=100000000000000,
            usd_price=0.0, usd_value=0.0,
            derivation_path="m/44'/60'/0'/0/0",
        )
        with patch(
