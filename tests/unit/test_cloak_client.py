...
def mock_env(monkeypatch):
    monkeypatch.setenv("CLOAKBROWSER_CDP_WS", "ws://127.0.0.1:9222/devtools/browser/xyz")
...
async def test_cloak_scraper_get_page_cdp(mock_env):
    ...
    with patch("src.core.cloak_client.async_playwright") as mock_playwright:
        ...
        mock_pw_context.chromium.connect_over_cdp.assert_called_once_with("ws://127.0.0.1:9222/devtools/browser/xyz")
...
async def test_cloak_scraper_force_cloak_fails_without_cdp(monkeypatch):
    ...
    with patch("src.core.cloak_client.async_playwright") as mock_playwright:
        ...
        with pytest.raises(RuntimeError, match="CloakBrowser is forced but no CDP WebSocket URL could be resolved"):
